...
(function () {
  ...
  // ---------- AI-style document-type classification (keyword scoring, not a trained model) ----------
  const detectTypeBtn = document.getElementById("detectTypeBtn");
  if (detectTypeBtn) {
    detectTypeBtn.addEventListener("click", function () {
      const text = docText.value.trim();
      if (!text) { docText.focus(); return; }
      const ranked = DocAnalysis.classifyDocumentType(text);
      const best = ranked[0];
      const original = detectTypeBtn.textContent;
      if (!best || best.score === 0) {
        detectTypeBtn.textContent = "🤔 Unclear";
      } else {
        docTypeSelect.value = best.id;
        detectTypeBtn.textContent = "✅ " + best.name;
      }
      setTimeout(function () { detectTypeBtn.textContent = original; }, 2500);
    });
  }
  ...
  document.getElementById("analyzeBtn").addEventListener("click", function () {
    ...
    const entities = DocAnalysis.extractNamedEntities(text);

    lastAnalysis = { text: text, docTypeName: docTypeName, stats: stats, dates: dates, payments: payments, obligations: obligations, outline: outline, risky: risky, missing: missing, summary: summary, entities: entities };
    ...
    const entityGroups = [
      { label: "Defined parties", items: entities.definedTerms },
      { label: "Persons", items: entities.persons },
      { label: "Organizations", items: entities.organizations },
      { label: "Locations", items: entities.locations }
    ].filter(function (g) { return g.items.length; });
    document.getElementById("entitiesResults").innerHTML = entityGroups.length
      ? entityGroups.map(function (g) {
          return '<div class="doc-extract-item"><strong>' + escapeHtml(g.label) + "</strong><p class=\"small\">" +
            g.items.map(escapeHtml).join(", ") + "</p></div>";
        }).join("")
      : '<p class="small muted">No persons, organizations, or Indian locations detected by our patterns.</p>';
  ...
  document.getElementById("generateReportBtn").addEventListener("click", function () {
    ...
    area.innerHTML =
      ...
      "<h2>Named Entities</h2>" + (document.getElementById("entitiesResults").innerHTML || "<p>None detected.</p>") +
